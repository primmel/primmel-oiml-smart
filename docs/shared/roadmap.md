...
| Twin certification program (TCD-1–TCD-6) | ● | TODO.v2/01, 2026-07-28, the `oiml-twin-cert` package (kind `certification_program`, type 5), the probe channel (three-source provenance, declared attestation limitation), the verdict chain + twin certificate (hard type-cert gate, suspension walk), the surveillance monitor binding + zero-gateway re-judgment SOP, the four-scenario acceptance against the live sim (5 legs), the TW-1 governing document with cite-integrity, ch 17 (smart `d2a2024`, kernel C99) |
