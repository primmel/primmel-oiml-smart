...
> Astro + Vue islands shell, and the command gates that keep every claim
> in this tree honest.
...
The browser app (`browser/`, Vue 3 + TypeScript) renders the
Recommendation content and runs the entire certification workflow from
that data: application, dispatch, testing, evaluation, certificate,
register (Annex A). What follows is how the engine is built.
...
## B.7 The Astro + Vue islands shell

The shell is **Astro 7 with Vue islands** (`browser/astro.config.mjs`);
the Vue SPA entry is gone. The load-bearing facts
(`docs/architecture.md` §14):

- **Hybrid output** — `output: 'server'` + `@astrojs/node` standalone.
  Static shells carry `export const prerender = true`; dynamic entity
  routes are `prerender = false` and render on demand (ids are not
  build-time enumerable; islands read params from the URL, never from
  `Astro.props`).
- **Generated routes** — `browser/src/pages/` contains only `.astro`
  files, all generated by `browser/scripts/generate-astro-routes.ts`
  (`npm run codegen:routes`: 194 route files); never edit the shells by
  hand. Vue page components live in `browser/src/vue-pages/`.
- **Two generic islands** — `src/astro/AppPage.vue` (Gate → AppShell →
  page) and `src/astro/PublicPage.vue` (PublicShell → page), lazy-loading
  the page from an `import.meta.glob` registry — one chunk per page.
- **The guard** — `src/astro/Gate.vue` runs `bootstrap()` + the session
  check per page: unauthenticated → `/app/login?redirect=…`, role
  mismatch → role home.
- **The vue-router shim is permanent** — `src/astro/app-entrypoint.ts`
